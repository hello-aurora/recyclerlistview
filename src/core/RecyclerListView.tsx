        ...
        if (this._scrollComponent) {
            ...
            if (relativeIndex > -1) {
                ...
                if (preserveVisiblePosition && layoutManager) {
                    ...
                    if (animate) {
                        // the amount of time taken for the animation is variable on ios, the animation
                        // is documented to be 'constant rate' at an unspecified rate, so the time is
                        // proportional to the length of scroll on android, the only relevant information
                        // the author has discovered is that default animation duration is 250ms. Therefore,
                        // we hold until relativeIndex comes into view + a little time (especially for
                        // low-end devices) such that all scroll events have fired
...
RecyclerListView.propTypes = {
    ...
    // Enables preserving calculated layouts on data changes; suitable if changes are mostly new items at edges,
    // rather than modifications which change sizes of existing items
