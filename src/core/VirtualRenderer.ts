...
export default class VirtualRenderer {
  ...
  private _layoutProvider: BaseLayoutProvider =
    TSCast.cast<BaseLayoutProvider>(null); //TSI
  ...
  public refreshWithAnchor(): void {
    if (this._viewabilityTracker) {
      let firstVisibleIndex =
        this._viewabilityTracker.findFirstLogicallyVisibleIndex();
  ...
  public syncAndGetKey(
    ...
    keyToStableIdMap?: { [key: string]: string },
  ): string {
  ...
  public handleDataSetChange(
    ...
    if (this._preserveVisiblePosition && this._layoutManager) {
      const startEdgeThreshold =
        this._layoutManager.getLayouts()[0].y + this._edgeVisibleThreshold;

      preservePosition =
        holdStableId !== undefined ||
        this._startEdgePreserved ||
        scrollOffset > startEdgeThreshold;
    ...
    for (let i = 0; i < oldActiveStableIdsCount; i++) {
      ...
      if (stableIdItem) {
        if (!(key in activeStableIds)) {
          ...
          if (
            !shiftLayouts &&
            !ObjectUtil.isNullOrUndefined(dataIndex) &&
            dataIndex <= maxIndex &&
            this._layoutManager
          ) {
    ...
    for (const key in this._renderStack) {
      if (this._renderStack.hasOwnProperty(key)) {
        ...
        if (
          ...
          ObjectUtil.isNullOrUndefined(this._engagedIndexes[index])
        ) {
          const type = this._layoutProvider.getLayoutTypeForIndex(index);
    ...
    if (this._layoutManager) {
      ...
      if (preservedStableId !== null) {
        ...
        if (shiftPreservedIndex !== -1) {
          if (preservePosition) {
            this._layoutManager.shiftPreservedIndex(
              preservedIndex,
              shiftPreservedIndex,
            );
          }
          if (shiftLayouts) {
            this._layoutManager.shiftLayouts(
              shiftPreservedIndex - preservedIndex,
            );
          }
        }
      }
    ...
    if (this._isRecyclingEnabled) {
      for (let i = 0; i < count; i++) {
        ...
        if (this._params && disengagedIndex < this._params.itemCount) {
          ...
          resolvedKey =
            this._stableIdToRenderKeyMap[this._fetchStableId(disengagedIndex)];
