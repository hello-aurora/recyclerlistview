...
export abstract class LayoutManager {
  ...
  public abstract overrideLayouts(
  ...
  public abstract preserveIndexes(
  ...
  public abstract shiftPreservedIndex(
    index: number,
    shiftPreservedIndex: number,
  ): void;
...
export class WrapGridLayoutManager extends LayoutManager {
  ...
  public relayoutFromIndex(startIndex: number, itemCount: number): void {
    ...
    if (startIndex <= this._fixIndex) {
      for (
        ;
        index < Math.min(oldItemCount - 1, itemCount - 1, this._fixIndex);
        index++
      ) {
  ...
  public refix(
    ...
    if (this._pendingRelayout) {
      ...
      if (
        scrollHeight <
        Math.min(scrollOffset, this._totalHeight) + refixOffset
      ) {
        innerScrollComponent.setNativeProps({
          style: { height: this._totalHeight + refixOffset },
        });
        innerScrollComponent.measure(
          (
            x: number,
            y: number,
            width: number,
        ...
        if (refixOffset !== 0) {
          ...
          if (Math.abs(refixOffset) >= 1) {
            innerScrollComponent.setNativeProps({
              style: { height: this._totalHeight },
            });
  ...
  private _preparePreservedIndex = (
    ...
    firstEngagedIndex: number,
    lastEngagedIndex: number,
  ): void => {
    let index = -1;
  ...
  private _checkBounds(
    ...
    return isHorizontal
      ? itemY + itemDim.height <= this._window.height + 0.9
      : itemX + itemDim.width <= this._window.width + 0.9;
  }
}
