...
export default class BinarySearch {
  ...
  public static findClosestValueToTarget(
    values: number[],
    target: number,
  ): ValueAndIndex {
    let low = 0;
   ...
   */
  public static findValueSmallerThanTarget(
    values: number[],
    target: number,
  ): ValueAndIndex | undefined {
    const low = 0;
    ...
    const midValueAndIndex: ValueAndIndex = this.findClosestValueToTarget(
      values,
      target,
    );
   ...
   */
  public static findValueLargerThanTarget(
    values: number[],
    target: number,
  ): ValueAndIndex | undefined {
    const low = 0;
